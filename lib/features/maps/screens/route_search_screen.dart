...
class _RouteSearchScreenState extends State<RouteSearchScreen> {
  ...
  List<dynamic> buses = [];
  bool isLoading = true;
  String selectedLeaveTime = DateFormat("hh:mm a").format(DateTime.now());
  ...
  Future<void> loadDirection() async {
    ...
    try {
      ...
      final destination = LocalGeocodingService.getCoordinates(
        destinationLocation,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "Unable to find a route. Please try another location.",
            ),
          ),
        );
      }
  ...
  List<Widget> _buildRouteCards() {
    if (!hasSearched) {
      return [
        const Padding(
          ...
          child: Center(child: Text("Enter destination to search routes")),
        ),
      ];
    }
    ...
    if (_selectedTransportIndex == 2) {
      ...
      filteredBuses = filteredBuses.where((bus) {
        return bus['source'].toString().toLowerCase().trim() ==
            sourceLocation.toLowerCase().trim();
      }).toList();

      filteredBuses = filteredBuses.where((bus) {
        return bus['destination'].toString().toLowerCase().trim() ==
            destinationLocation.toLowerCase().trim();
      }).toList();

      print("After source/destination filter = ${filteredBuses.length}");
      ...
      filteredBuses = filteredBuses.where((bus) {
        final busTime = DateFormat(
          "hh:mm a",
        ).parse(isArriveSelected ? bus['arrivalTime'] : bus['departureTime']);

        final selectedTime = DateFormat("hh:mm a").parse(selectedLeaveTime);

        return !busTime.isBefore(selectedTime);
      }).toList();
      ...
      if (selectedFilter == "Fewer transfers") {
        ...
        filteredBuses.sort(
          (a, b) =>
              (a['transferCount'] ?? 0).compareTo(b['transferCount'] ?? 0),
        );
      }
      ...
      if (selectedFilter == "Less walking") {
        ...
        filteredBuses.sort(
          (a, b) =>
              (a['walkingDistance'] ?? 0).compareTo(b['walkingDistance'] ?? 0),
        );
      }
          ...
          children: [
            RouteCard(
              data: RouteCardData(
                ...
                scheduleInfo: "${bus['source']} → ${bus['destination']}",
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) => RouteDetailsScreen(
                      ...
                      walkingDistance: (bus['walkingDistance'] as num)
                          .toDouble(),
          ...
          children: [
            ...
            Positioned.fill(
              child: RouteMapWidget(
                source: sourceLatLng ?? const LatLng(8.9598, 77.3152),
                destination: destinationLatLng ?? const LatLng(8.7139, 77.7567),
            ...
            Positioned(
              ...
              child: FloatingRouteSearchBar(
                source: sourceLocation,
                destination: destinationLocation,
                onSearch: (source, destination) async {
                  if (isLoadingRoute) return;
                  ...
                  child: CustomScrollView(
                    ...
                    slivers: [
                                  ...
                                  child: ListView.separated(
                                    ...
                                    itemBuilder: (context, i) {
                                      return TransportButton(
                                        icon: StaticRouteData
                                            .transportModes[i]
                                            .icon,

                                        label: StaticRouteData
                                            .transportModes[i]
                                            .label,

                                        isSelected:
                                            i == _selectedTransportIndex,

                                        iconFilled: StaticRouteData
                                            .transportModes[i]
                                            .iconFilled,
                      ...
                      SliverPadding(
                        ...
                        sliver: SliverList(
                          delegate: SliverChildListDelegate(_buildRouteCards()),
                        ),
                      ),
                    ],
                  ),
          ...
          child: Row(
            ...
            children: [
              Text(
                ...
                style: SmartNavTextStyles.labelLg.copyWith(fontSize: 12),
              ),
...
class _SheetIconButton extends StatelessWidget {
  ...
  const _SheetIconButton({required this.icon, required this.onTap, super.key});
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: SizedBox(
          ...
          child: Icon(icon, size: 22, color: SmartNavColors.onSurfaceVariant),
        ),
      ),
    );
  }
