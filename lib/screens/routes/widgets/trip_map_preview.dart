...
class TripMapPreview extends StatefulWidget {
  ...
  const TripMapPreview({super.key, required this.vehicleNumber});
...
class _TripMapPreviewState extends State<TripMapPreview> {
  ...
  Future<void> loadBusLocation() async {
    final response = await http.get(
      Uri.parse(
        "${ApiConfig.baseUrl}/api/locations/latest/${widget.vehicleNumber}",
      ),
    );
    ...
    if (mapController != null) {
      mapController!.animateCamera(
        CameraUpdate.newCameraPosition(
          CameraPosition(target: busLocation, zoom: 16),
        ),
      );
    }
  }
      ...
      child: GoogleMap(
        ...
        markers: {
          Marker(
            ...
            position: const LatLng(8.9598, 77.3152),
          ...
          Marker(
            ...
            infoWindow: InfoWindow(title: widget.vehicleNumber),
          ),

          Marker(
            markerId: const MarkerId("destination"),
            position: const LatLng(8.7139, 77.7567),
        ...
        polylines: {
          Polyline(
            ...
            color: Colors.blue,
            width: 5,
            points: const [LatLng(8.9598, 77.3152), LatLng(8.9700, 77.3300)],
          ),
        },

        onMapCreated: (controller) {
          mapController = controller;
          controller.animateCamera(CameraUpdate.newLatLngZoom(busLocation, 14));
        },
      ),
    );
  }
